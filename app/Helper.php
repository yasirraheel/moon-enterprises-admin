...
class Helper
{
	...
	{
		// Convert to float and handle potential null values
		$currentPeriod = (float) $currentPeriod;
		$previousPeriod = (float) $previousPeriod;
		
		// Handle edge cases where both values are zero (including very small floating point numbers)
		if (abs($currentPeriod) < 0.0001 && abs($previousPeriod) < 0.0001) {
			return '<small class="float-end text-muted">
		<strong>0%</strong>
		</small>';
		}
		
		// Handle case where previous period is zero but current is not
		if (abs($previousPeriod) < 0.0001 && $currentPeriod > 0.0001) {
			return '<small class="float-end text-success">
		<strong><i class="bi bi-arrow-up me-1"></i> 100%</strong>
		</small>';
		}
		
		// Handle case where current period is zero but previous is not
		if (abs($currentPeriod) < 0.0001 && $previousPeriod > 0.0001) {
			return '<small class="float-end text-danger">
		<strong><i class="bi bi-arrow-down me-1"></i> 100%</strong>
		</small>';
		}
		
		// Additional safety check to prevent division by zero
		if (abs($previousPeriod) < 0.0001) {
			return '<small class="float-end text-muted">
		<strong>0%</strong>
		</small>';
		}
		
		// Calculate percentage change when both values are non-zero
